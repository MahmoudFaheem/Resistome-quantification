...
#4. Use reformat from the BBmap suite to interleave the generated forward and reverse fastq files, R_1 and R_2, and generate a single interleaved fastq file for further analysis.
reformat.sh in1=~/fastq/${R1} in2=~/fastq/${R2} out=interleaved${srr_id}.fq.gz

#5. Reads alignment with Bowtie2 to a reference database containing antimicrobial resistance gene sequences
...
#8. A single-line script to extract the gene name only and make the header tab-separated. Also, it creates a new column called "Gene_Count" and assigns a value of +1 to each gene every time it appears. It provides four files, diamond, awk_diamond, gene_sums [showing the number of occurrences for each unique gene], and gene_counts [which is just the total number of all genes combined]:
