...
#8. 20240112_New_adjustment (A single line script to extract the gene name only and make the header tab-separated, it also creates a new column called "Gene_Count" and assigns a value of +1 to each gene every time it appears. It provides four files, diamond, awk_diamond, gene_sums [showing the number of occurrences for each unique gene], and gene_counts [which is just the total number of all genes combined]:
